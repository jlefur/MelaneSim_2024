...
public abstract class A_Animal extends A_Organism implements I_ConstantNumeric {
	...
	protected double evalScarcity() {
		int nbAnimals = 0, nbPartners = 0;
		TreeSet<I_SituatedThing> accointanceList = this.perception();
		for(I_SituatedThing accointance:accointanceList){
			if(accointance instanceof A_Animal){
				nbAnimals++;
				if(((A_Animal)accointance).getGenome().getClass()==this.genome.getClass()) nbPartners++;
			}
		}
		return (double)nbPartners/(double)nbAnimals;
		/*
		 * // Procedure for hybridation enclosure: return a value between 0 and 1 where 0 means equal number of both
		 * species and 1 // scarcity f any of the species. double rateConspecific = (double) nbPartners / (double)
		 * nbRodent; double scarcity = java.lang.Math.abs((rateConspecific - .5) * 2); //
		 * System.out.println("C_Rodent.evalScarcity(): nat/total: "+nbNatalensis+"/"+nbRodent+"="+percentNat+"%, -> "
		 * +scarcity); return scarcity;
		 */
	}
	...
	protected boolean recognized(A_Animal partner) {// TODO JLF 2014.07 clean up for normal function
		double thisSignature = ((C_GenomeEucaryote)this.genome).hybridRatio();
		double partnerSignature = ((C_GenomeEucaryote)partner.genome).hybridRatio();
	...
	public void initParameters() {
		...
		// Get sensing surface
		double sensingSurface_USquareMeterByTick = ((C_GenomeAnimalia)this.genome).getSensingSurface_USquareMeterByDay()
		        /C_ConvertTimeAndSpace.oneDay_Utick;
	...
	public void actionEnterContainer(I_Container oneContainer) {
		...
		myLandscape.moveToLocation(this,myLandscape.getThingCoord_Ucs(oneContainer));// Place agent at the container
		                                                                             // position
	...
	/** If not dead and within a dangerous area, test death, get back two steps. Can be overridden by daughter
	 * classes<br>
	...
	protected void checkDanger() {
		...
		// If arrived in wrong place, get back two steps
			...
			else this.checkDeath(DANGEROUS_AREA_MORTALITY_RATE/4.);// TODO number in source 2019.02 JLF Reduce death
			                                                       // probability
			                                                       // for e.g. streets
		}
	}
	...
	public boolean canPerceiveThing(I_SituatedThing oneThing) {
		return(this.getDistance_Umeter(oneThing)-(C_Parameters.CELL_WIDTH_Umeter/2)<this.getSensing_UmeterByTick());
	}
	...
	/** If no next move, get a random one; then (TODO number in source 2017.09 JLF ) actionMove REPEAT_DISPERSAL times
	 * to accentuate dispersal <br>
	...
	/** Use list of perceived things and choose any container except A_Organism (eggs, relatives, ...) as a forage
	 * source<br>
	 * rev.JLF 02.2017, 01.2020 */
	public TreeSet<I_SituatedThing> chooseFood(TreeSet<I_SituatedThing> perceivedThings) {
		TreeSet<I_SituatedThing> candidateTargets = new TreeSet<I_SituatedThing>();// There may be several equivalent
		                                                                           // targets
	...
	/** Provide an aim to move toward specific coordinates. We watch if the agent is (or isn't) close to its goal, we
	 * have to do several tests because if the agent is near in the x variable but not in the y, the next step it may be
	 * too far on the x abscissa and near to the y abscissa, we have to make sure that the agent won't go on a perpetual
	 * move. author QBaduel */
	 ...
	 * @param agentSpeed_UmeterByTick is the max distance of the agent or agent's vehicle to go over
	 *            (speed_UmeterBy...). if speed_Umeter exceeds the targetPoint then the distance to target is just the
	 *            distance between currentPoint_Umeter and targetPoint_Umeter. <br>
	 *            Le d�placement est suivant un vecteur de coordonn�es (vectCoordX, vectCoordY) ie targetPoint_Umeter -
	 *            currentPoint_Umeter (sens), de longeur vectNorm = racine(vectCoordX�, vectCoordY�) (norme) et faisant
	 *            un angle alpha par rapport � l'abscisse (direction) si vectCoordX == 0, alpha = +PI/2 ou -PI/2 �a
	 *            d�pend du signe de vectCoordY sinon alpha = atan(vectCoordY/vectCoordX) Ainsi nous avons la direction
	 *            et le sens du d�placement. Et donc � partir de la distance � parcourir, on peut calculer exactement
	 *            nextMove_Umeter.x et nextMove_Umeter.y
	 ...
	 * @param agentSpeed_UmeterByTick is the max distance of the agent or agent's vehicle to go over
	 *            (speed_UmeterBy...). if speed_Umeter exceeds the targetPoint then the distance to target is just the
	 *            distance between currentPoint_Umeter and targetPoint_Umeter.
	 * @see #computeNextMoveToTarget()
	...
	protected void computeNextMoveToTarget0(double agentSpeed_UmeterByTick) {
		if(this.target==null)
		    ...
		    if(agentSpeed_UmeterByTick>0 && !trappedOnBoard){
			    ...
			    this.nextMove_Umeter = C_VariousUtilities.computeVectorBetweenTwoPoints(this.getCoordinate_Umeter(),this
			            .getTargetPoint_Umeter(),agentSpeed_UmeterByTick);
		    }
	}
	...
	public boolean isArrived() { return this.isArrived(this.speed_UmeterByTick); }
	public boolean isHasToLeaveFullContainer() { return this.hasToLeaveFullContainer; }
	public boolean isTrappedOnBoard() { return trappedOnBoard; }
	...
	public Coordinate getTargetPoint_Umeter() { return this.target.getCoordinate_Umeter(); }
	public I_SituatedThing getTarget() { return this.target; }
