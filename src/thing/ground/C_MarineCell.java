...
public class C_MarineCell extends C_SoilCell implements I_ConstantPNMC_particules {
	...
	public void step_Utick() {
		...
		if (this.speedEastward_UmeterPerSecond + this.speedNorthward_UmeterPerSecond == 0.)
			return;
		else {
			TreeSet<I_SituatedThing> occupants = new TreeSet<>(this.getOccupantList());
			// Object[] occupants = this.getOccupantList().toArray();// To avoid concurrent
			// exception
			for (I_SituatedThing agent : occupants) {
				if (agent instanceof C_Plankton) {
					speedEastward_UmeterPerTick = C_ConvertTimeAndSpace
							.convertSpeed_UspaceByTick(this.speedEastward_UmeterPerSecond, "m", "s");
					speedNorthward_UmeterPerTick = C_ConvertTimeAndSpace
							.convertSpeed_UspaceByTick(this.speedNorthward_UmeterPerSecond, "m", "s");
					A_VisibleAgent.myLandscape.translate((A_VisibleAgent) agent,
							new Coordinate(speedEastward_UmeterPerTick, speedNorthward_UmeterPerTick));
					energy = Math.sqrt(speedEastward_UmeterPerTick * speedEastward_UmeterPerTick
							+ speedNorthward_UmeterPerTick * speedNorthward_UmeterPerTick) / 1E3;// TODO number in
																									// source JLF 2024
																									// (energy = speed
																									// /1E3)
