 ...
 * Values are read from a file. It can be an ASCII text grid or an image raster. (the image must be in grey levels or in
 * 256 or less colors). affinity values are stored in a gridValueLayer (as well as in the cell container attributes)
 * <br>
...
public class C_Landscape implements I_ConstantString {
	...
	protected static Map<Integer,Color> colorMap;// color associated to each different value of the raster or text grid
	                                             // file
	//
	// CONSTRUCTOR
	//
	/** Constructor of grid ground : creates a gridValueLayer with values (affinity); an equivalent matrix with field
	 * agents, a continuous space where agents have real coordinates. Retrieves also for that the user parameters
	 * provided within the GUI */
	public C_Landscape(Context<Object> context,String url,String gridValueName,String continuousSpaceName) {
		...
		A_Protocol.event("C_Landscape constructor","dimensions dim-ucell: width = "+dimension_Ucell.width+" height = "
		        +dimension_Ucell.height,isNotError);
		...
		this.gridValueLayer = new GridValueLayer(gridValueName,true,new repast.simphony.space.grid.WrapAroundBorders(),
		        dimension_Ucell.width,dimension_Ucell.height);
		context.addValueLayer(this.gridValueLayer);
		A_Protocol.event("C_Landscape constructor","dimensions gridValueLayer ("+gridValueName+"): "+gridValueLayer
		        .getDimensions(),isNotError);
	...
	/** Addition of new agent in simulation. The agent is not sexed in this method. And get the position of it's
	 * "parents".
	 ...
	 * This method deals with gridcells (i.e., C_SoilCell); we convert the vision of the agent from a vision in meter to
	 * a vision in cells then we obtain the field agent associated to the concerned cells. We then keep only those
			...
			for(int j = minY_Ucell; j<=maxY_Ucell; j++){
				if(i>=0 && j>=0 && i<gridWidth_Ucell && j<gridHeight_Ucell && ((A_Animal)thing).canPerceiveThing(
					...
					for(I_SituatedThing oneOccupant:grid[i][j].getFullOccupantList()){
						/** Agent restricts cellsContent to acquaintances strictly within their sensing radius - LeFur
						 * 2011 */
						if(continuousSpace.getLocation(oneOccupant)!=null){ // Patch: occurs when border drives agent
						                                                    // out
							if(thing.canInteractWith(oneOccupant)) surroundingObjects.add((I_SituatedThing)oneOccupant);
						}
					}
				}
			}
	 ...
	 * CAUTION This method is a patch : using this procedure should be accompanied with the various list update
	 * necessary (see previous uses for suggestion) Version JEL/JLF 2011, rev. PAM 2013, 06.2015, jlf 12.2015 @param
	 * thing I_SituatedThing to move
	...
	/** Move the agent on the continuous space p_Ucell.getX() + (int) (distanceDeplacement_Ucs.x / SPACE_CELL_SIZE_UCS),
	 * Remark: The ground manager may change the requested move to match the field constraint Link between raster and
	 * continuous space<br>
	...
	/** Results in the exit of an agent and the entry of a new one in a random point at the edges of the simulation
	 * space. author Longueville 2011, rev. JLF 11.2015
	...
	protected void replaceOutcomer(A_VisibleAgent animalLeavingLandscape, double[] newLocation, int x, int y) {
		...
		Context<I_SituatedThing> context = ContextUtils.getContext(animalLeavingLandscape);// Keep context to introduce
		                                                                                   // incomer
		...
		/*
		 * if (C_Parameters.VERBOSE) A_Protocol.event("C_Landscape:bordure", "BORDURE event: " + animalLeavingLandscape
		 * + " leaves landscape at " + animalLeavingLandscape.getCurrentSoilCell().retrieveLineNo() + "," +
		 * animalLeavingLandscape .getCurrentSoilCell().retrieveColNo() + " / " + incomer + " enters at " + x + "," + y,
		 * isNotError);
		 */
	}
							...
							for(y = y0-1; y<=y0+1; y++){ // avec ces 2 boucles j'accede � tous les 8 voisins de sc0
								if(!(x==x0 && y==y0) && (0<=x && x<dimension_Ucell.getWidth() && 0<=y
									...
									if(scI.getAffinity()==affinity0 && // si c'est de la m�me affinit� que sc0 et
										...
										fileDattente.add(scI); // et l'ajoute dans la file d'attente pour �tudier � son
										                       // tour
	 ...
	 * @return map */
	public static Map<Integer,Color> getColormap() { return colorMap; }
	...
	public void setGridCell(int line, int col, I_Container cell) { this.grid[line][col] = cell; }
	public I_Container getGridCell(int line, int col) { return this.grid[line][col]; }
	public GridValueLayer getValueLayer() { return gridValueLayer; }
	public Dimension getDimension_Ucell() { return dimension_Ucell; }
	public TreeSet<C_LandPlot> getAffinityLandPlots() { return landPlots; }
	public ContinuousSpace<I_SituatedThing> getContinuousSpace() { return continuousSpace; }
}