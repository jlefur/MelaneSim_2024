...
public class C_VariousUtilities implements I_ConstantStringRodents {
	...
	/** Compute and return a movement vector between two point according to a verification distance Le d�placement est suivant un
	 * vecteur de coordonn�es (vectCoordX, vectCoordY) ie fisrtPoint - secondPoint (sens), de longeur vectNorm =
	 * racine(vectCoordX�, vectCoordY�) (norme) et faisant un angle alpha par rapport � l'abscisse (direction) si vectCoordX == 0,
	 * alpha = +PI/2 ou -PI/2 �a d�pend du signe de vectCoordY sinon alpha = atan(vectCoordY/vectCoordX) Ainsi nous avons la
	 * direction et le sens du d�placement. Et donc � partir de la distance � parcourir, on peut calculer exactement le vecteur de
	 * d�placement */
