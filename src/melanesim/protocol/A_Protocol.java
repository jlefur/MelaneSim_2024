...
public abstract class A_Protocol implements I_Protocol, I_ConstantString, I_ConstantNumeric {
	...
	/** In short: make this method first inherit its daughters at startup, then proceed to super (which is not the case
	 * within the constructor (super must be first there)<br>
	 * Longer: Triggered by the context creator at the beginning of the simulation and after constructing the
	 * protocol<br>
	 * The procedure can be realized only after all the CASCADING INSPECTORS and headers have been defined in the
	 * subclass hierarchy i.e., contains procedure that cannot be put in the constructor to avoid the overload when
	 * subclasses use super in their constructor */
	 ...
	 * Check sizes of the rodents'list, manage time stuff, proceed to all NDSs' step, updates indicators, remove dead
	 * agents<br>
	 ...
	 * To avoid concurrent modification exception as well as scrambling the order of rodents, all death procedure mark
	 * the rodent as having to die. The death and remove of dead rodents is then proceeded in one shot <br>
	...
	/** Destroy a thing, remove it from context and remove its references to other object (lastContainerLeft,
	 * inspectors) so as to be garbage collected<br>
	...
	@Override
	/** Read user parameters on the GUI, check if temporal scale and cell size has changed, reset timeConverter if
	 * needed<br>
	 * rev. jlf 09.2017 */
	public void readUserParameters() {
		...
		if((C_Parameters.TICK_LENGTH_Ucalendar!=oldTickLength) || (C_Parameters.TICK_UNIT_Ucalendar!=oldTickUnit)){
			...
			A_Protocol.event("A_Protocol.readUserParameters","New tick definition: "+C_Parameters.TICK_LENGTH_Ucalendar
	...
	protected void haltSimulation() {
		...
		A_Protocol.event("A_Protocol.haltSimulation","Simulation length: "+((int)simLength/1000)+"sec. (~"
		        +((int)simLength/60000)+"mn, ~"+((int)simLength/3600000)+"h.) / Tick "+(int)RepastEssentials
		                .GetTickCount()+".",isNotError);
	...
	public static String event(String source, String message, Boolean isError) {
		message = protocolCalendar.stringShortDate()+" (tick "+(int)RepastEssentials.GetTickCount()+"): "+message;
	...
	public void printContextFullContent() {
		...
		for(int i = 0; i<list.length; i++) System.out.println("�l�ment: "+C_VariousUtilities.getShortClassName(list[i]
		        .getClass())+": "+list[i]);
	...
	// SETTER & GETTERS
	//
	public void setLandscape(C_Landscape landscape) { this.landscape = landscape; }
