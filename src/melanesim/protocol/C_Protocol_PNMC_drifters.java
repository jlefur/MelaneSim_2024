...
public class C_Protocol_PNMC_drifters extends A_Protocol implements I_ConstantPNMC {
	...
	/**
	 * Declare the inspectors, add them to the inspector list, declare them to the
	 * panelInitializer for indicators graphs<br>
	...
	/**
	 * Add plankton particle in the center of each cell of the grid at a specified
	 * interval - JLF 07.2024
	 */
	...
	public void initProtocol() {
		this.geographicCoordinateConverter = new C_ConvertGeographicCoordinates(
				new Coordinate(I_ConstantPNMC.rasterLongitudeWest_LatitudeSouth_Udegree.get(0),
	...
	protected void initLandscape(Context<Object> context) {
		this.setLandscape(
				new C_LandscapeMarine(context, C_Parameters.RASTER_URL, VALUE_LAYER_NAME, CONTINUOUS_SPACE_NAME));
	...
	public void manageTimeLandmarks() {
		...
		if (displayMapBefore != C_Parameters.DISPLAY_MAP)
			switchDisplayMap();
	...
	public void saveScreen() { /**
								 * // Uncomment lines below to slightly randomly move the mouse to avoid screen
								 * sleep mode (for recording printscreen) try { Robot robot = new Robot(); //
								 * Get the current mouse coordinates Point mouseLocation =
								 * MouseInfo.getPointerInfo().getLocation(); int x = mouseLocation.x; int y =
								 * mouseLocation.y; robot.mouseMove(x + random.nextInt(3), y +
		...
		String currentDate = "Energy-" + currentYear + "." + String.format("%02d", currentMonth + 1)
				+ String.format("%03d", currentDay) + "_" + String.format("%02d", currentHour);
	...
	@Override
	/**
	 * Display the map if on, remove it if off. Only one map object. The switch can
	 * only go from on to off and vice versa Version author J.Le Fur, 09.2014
	 */
