...
import thing.rodents.C_TaxiMan;
import data.C_Event;
...
public abstract class A_ProtocolTransportation extends A_ProtocolRodent implements I_ConstantTransportation {
	...
	public void step_Utick() {
		// inspectorTransportation.checkLandPlotLists();// TODO JLF Slow simulation however, useful for some undetected
		// ;-) bugs
		removeRatSurplus();
	...
	public void readUserParameters() {
		...
		C_Parameters.HUMAN_SUPER_AGENT_SIZE = ((Integer)C_Parameters.parameters.getValue("HUMAN_SUPER_AGENT_SIZE"))
		        .intValue();
		// DEFAULT VALUES
		double oldLoadingProb = C_Parameters.VEHICLE_LOADING_PROBA_DIVIDER;// store the old value to detect if it has
		                                                                   // changed
		...
		if(C_Parameters.VEHICLE_LOADING_PROBA_DIVIDER!=oldLoadingProb && RepastEssentials.GetTickCount()>=0){
			...
			for(C_HumanCarrier carrier:inspectorTransportation.getCarrierList()) carrier.getVehicle().initParameters();
		...
		if((C_Parameters.TICK_LENGTH_Ucalendar!=oldTickLength) || (C_Parameters.TICK_UNIT_Ucalendar!=oldTickUnit)){
	...
	protected void manageReadEventTypes(TreeSet<String> changedEventTypes) {
		...
		for(String eventType:changedEventTypes){
			...
			if(GROUND_TYPE_CODES.containsKey(eventType)){// TODO JLF 2014.09 patch for centenal GNT (eventTypes are
			                                             // added in
				...
				if(((C_LandscapeNetwork)this.landscape).getAreaTypes().contains(eventType)){
					for(String areaType:((C_LandscapeNetwork)this.landscape).getAreaTypes()){
						// 1) sets the landPlot list from the rasterGraph 2) include the changes in the existing
						// landplots
				...
				for(C_HumanCarrier oneCarrier:inspectorTransportation.getCarrierList()){
					if(eventType.equals(oneCarrier.getVehicle().getGraphType())){
						TreeSet<C_City> citiesToTrade = getCitiesToTrade(oneCarrier.getCitiesToTradeInfo_Ustring(),
						        (C_SoilCellGraphed)oneCarrier.retrieveMyHome(),oneCarrier.getVehicle().getGraphType());
						if(!citiesToTrade.isEmpty()) oneCarrier.setCityList(citiesToTrade);
						    ...
						    A_Protocol.event("A_ProtocolTransportation.manageReadEventTypes",oneCarrier+" in "
						            +oneCarrier.getCurrentSoilCell()+" cannot reset city list for "+oneCarrier
						                    .getCitiesToTradeInfo_Ustring()+", keeps current: "+oneCarrier.getCityList()
						                            .size()+" cities",isError);
					}
				}
	...
	protected void manageOneEvent(C_Event event) {
		...
		switch(event.type){
			...
			case TRACK_EVENT:
				...
				if(eventSC.isOfGroundType(GOOD_TRACK_EVENT)) eventSC.getGroundTypes().remove(GOOD_TRACK_EVENT);// Case
				                                                                                               // when a
				                                                                                               // good
				                                                                                               // track
				                                                                                               // is
				                                                                                               // abandoned
				if(eventSC.isOfGroundType(ROAD_EVENT)) eventSC.getGroundTypes().remove(ROAD_EVENT);// Case when a good
				                                                                                   // track is
				                                                                                   // abandoned
	...
	/** 1/add the ground type to the event soil cell, 2/sets the ground type value in valuelayer2 (the map of
	 * groundtypes)
	...
	protected void manageGroundEvent(C_Event event) {
		...
		eventSC.getGroundTypes().add(eventType);// Assert ground type of the soilCell (one soilCell can have many
		                                        // groundType)
		if(eventType.equals(CITY_EVENT)) eventSC.getGroundTypes().add(TRACK_EVENT);// a market cell is also managed as a
		                                                                           // city
		                                                                           // cell
		if(eventType.equals(CITY_EVENT)) eventSC.getGroundTypes().add(ROAD_EVENT);// a market cell is also managed as a
		                                                                          // city cell
		if(eventType.equals(CITY_EVENT)) eventSC.getGroundTypes().add(RAIL_EVENT);// a market cell is also managed as a
		                                                                          // city cell
		...
		if(eventType.equals(CITY_EVENT) || !eventSC.getGroundTypes().contains(CITY_EVENT)){
			if(event.value1.length()!=0){
				if(event.value2.length()!=0)
				    eventSC.setThisName(event.value2+"["+event.value1+"]"+NAMES_SEPARATOR+eventSC.retrieveId());
				else
		...
		if((currentGroundTypeColor==GROUND_TYPE_CODES.get(GOOD_TRACK_EVENT)) && (eventGroundTypeCode==GROUND_TYPE_CODES
		        .get(TRACK_EVENT))){
	 ...
	 * @param event The event in this case is of the form: type of vehicle/number of human carriers to add/option:
	 *            cities, area of activity / author PA Mboup 2014, rev. JLF 09.2014, 10.2014 */
	protected void manageVehicleEvent(C_Event event) {
		...
		// CHECK End
		    ...
		    // if creation is within a city
		    if((eventSC.getLandPlot(TOWN_EVENT)!=null) || (eventSC.getLandPlot(CITY_EVENT)!=null)){
			    ...
			    // the set of cities(/accointances)known
			    TreeSet<C_City> citiesToTrade = getCitiesToTrade(event.value2,eventSC,graphType);
			    ...
			    if(graph==null)
			        System.err.println("A_ProtocolTransportation.manageVehicleEvent(), no "+graphType+" graph for this "
			                +vehicleType+" event in "+eventSC);
		        ...
		        A_Protocol.event("A_ProtocolTransportation.manageVehicleEvent","No city at: "+eventSC+", "+nCarriers
		                +" not created",isError);
	}
	/** Determine the list of cities or markets to trade given the content of eventValue2<br>
	 * In the case where there is a specific list of cities to trade, event value2 contains a list of cities in the form
	 * tag 'city' followed with ':city1:city2...' */
	public TreeSet<C_City> getCitiesToTrade(String eventValue2, C_SoilCellGraphed eventSC, String graphType) {
		...
		if(eventValue2.length()!=0){
			...
			if((tagOfValue2.equals(CITY_EVENT) || (tagOfValue2.equals(TOWN_EVENT)))){// Convert the string into a list
			                                                                         // of C_City
	...
	protected void createCarrier(C_SoilCell cell, String vehicleType, C_Graph graph, TreeSet<C_City> citiesToTrade,
		...
		if(vehicleType.equals(TRUCK_EVENT))
		    for(C_City oneCity:citiesToTrade) if(!(oneCity instanceof C_Market)) oneCarrier.getCityList().add(oneCity);
	...
	/** Update the list of all landPlots in the groundType in parameter: the code compares the old landplot list and the
	 * new computed landplot list. Can update, shorten or extend landPlot and group several landPlots in one single; can
	 * update a landPlot divided into two or several (divide the road into several roads is to shorten it and add
	 * others); but can not yet destroy a road that does not exist anymore. <br />
	 * This method should only be called after building or rebuilding a groundType landPlots with
	 * identifyTypeLandPlots() method with new landPlots from identifyTypeLandPlots(), this method update the old one
	 * and add news. <br />
	 * An old landPlot and a new are the same if 2 conditions : 1- they get a same groundType; 2- they own a cell in
	 * common <br/>
	...
	protected void updateLandPlots(TreeSet<C_LandPlot> newLandPlots, String groundType) {
		...
		// If this ground type has no recorded landplot, a type is created and filled with the new land plot list,
		// nothing more.
		if(currentLandPlotsOfAType.size()==0){
			for(C_LandPlot aNewLandPlot:newLandPlots) createLandPlot(aNewLandPlot);
		}
					...
					for(C_SoilCell soilCell_1:oneOldLandPlot.getCells()){
						...
						if(oneNewLandPlot.getPlotType().equals(oneOldLandPlot.getPlotType()) && newLandPlotCells
							...
							if(updatedLandPlot==null){
								...
								// Rebuild the old landplot with the soil cell of the new one (keeping the references
								// agents know)
							...
							// else: one old landplot has already be updated, then soil cells have already been merged.
							// Therefore,
							// remove this one
							else{
								for(C_SoilCell sc:oneOldLandPlot.getCells()) if(((C_SoilCellGraphed)sc).getLandPlot(
								        groundType)==oneOldLandPlot){
	...
	public int removeCarrierPop(int nCarriers, C_SoilCellGraphed eventCell, String VehicleType,
		...
		// Pour d�truire des carriers sur un landPlot (de type route, rail, river ...)
		else{// remove carriers on the same landplot, select the landplot corresponding to the vehicleType (which is
		     // unique)
		...
		if(city.length!=0){
			...
			while(nbOfRodentsToDelete>0){
				...
				for(j = j0; j<cityCellSize; j++){ // For each cell
					j0++;
					// parce qu'il avait + 1 Ex: n= 15/4 =3 r=3; r!=0 => n=4. Dans les 3 1eres cell je mets 4, et quand
					// r==0 (4th
					// cell) je met 3(n-1)
					if(r==0) n = n-1;
					r--;
