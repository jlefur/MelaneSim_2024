...
public class C_ProtocolGerbil extends A_ProtocolFossorial implements I_ConstantGerbil {
		...
		switch (event.type) {
			case RAIN_EVENT :// file name example: 199901-PE-Rain.txt or 199901-TPE-Rain.txt
				...
				if (calendar.get(Calendar.MONTH) < 9)
					url = RASTER_PATH_RODENTS + rainUrl_suffixRainFile.get(0) + calendar.get(Calendar.YEAR) + "0" + (calendar
							.get(Calendar.MONTH) + 1) + rainUrl_suffixRainFile.get(1);
				else
					url = RASTER_PATH_RODENTS + rainUrl_suffixRainFile.get(0) + calendar.get(Calendar.YEAR) + (calendar.get(
	...
	/** TODO JLF 2021.02 temporaire pour r�duire la population */
