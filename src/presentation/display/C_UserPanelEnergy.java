...
public class C_UserPanelEnergy extends JPanel {
  ...
  public C_UserPanelEnergy() {
    ...
    setAll(
        safe(C_Parameters.CHLOROPHYLL_MULTIPLIER),
  ...
  private JPanel column(String name, JSlider slider, JSpinner spinner, Runnable sliderToSpinner) {
    ...
    // Sync spinner -> slider (applique direct si Live sinon on attend clic "Appliquer")
  ...
  private static double safe(double v) {
    if (Double.isNaN(v) || Double.isInfinite(v) || v < 0.25 || v > 2.0) return 1.0;
    return v;
  }
  ...
  private void applyToParameters() {
    // ⚠️ ne fais ici que des affectations légères ; si tu dois recalculer des trucs lourds,
