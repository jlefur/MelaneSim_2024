...
public class C_Style2dGroundType implements ValueLayerStyleOGL, I_ConstantNumeric, I_ConstantStringRodents,
	...
	public C_Style2dGroundType() {// TODO JLF 2014.11 rename into protocolColorMapFactory
		...
		if(this.colorMap==null){
			...
			/** Elaborating a random colored map colorMap = colorMapPattern55(colorMap); // we associate at each index
			 * between 0 and 255 a random color // ( we can equally decide to build our own colormap with specific //
			 * value. for (int i = 0; i < 256; i++) { colorMap.put(i, new Color( (int)
	...
	public Map<Integer,Color> colorMapPNMC_energyGrid(Map<Integer,Color> colorMap) {
		...
		// colorMap.put(ENERGY_RESET, new Color(231, 244, 255));
		// colorMap.put(ENERGY_LAND, new Color(2, 88, 0));
	...
	public Map<Integer,Color> colorMapCentenalGrid(Map<Integer,Color> colorMap) {
		...
		colorMap.put(GROUND_TYPE_CODES.get(RAIL_EVENT),new Color(200,200,200)); // RAIL (noir un peu moins fonc�)
		...
		colorMap.put(GROUND_TYPE_CODES.get(GNT_WEAK_EVENT),new Color(0,99,0)); // GROUND_NUT_TRADE_WEAK (GNT-WEAK) vert
																				// olive
																				// //171, 171, 57
		colorMap.put(GROUND_TYPE_CODES.get(GNT_MEDIUM_EVENT),new Color(153,255,0)); // GROUND_NUT_TRADE_MEDIUM
																					// (GNT-MEDIUM)
																					// plus petit
																					// (vers 1950) 206, 206, 48
		colorMap.put(GROUND_TYPE_CODES.get(GNT_HEAVY_EVENT),new Color(236,236,19)); // GROUND_NUT_TRADE_HEAVY
																					// (GNT-HEAVY) jaune
	...
	public Map<Integer,Color> colorMapMusTransportGrid(Map<Integer,Color> colorMap) {
		...
		colorMap.put(GROUND_TYPE_CODES.get(TOWN_EVENT),new Color(214,163,60)); // TOWN (marron clair (ou un peu plus
																				// fonc�:
																				// 237, 180, 66)
		colorMap.put(GROUND_TYPE_CODES.get(ROAD_EVENT),new Color(76,76,76)); // ROAD (gris fonc�)
		colorMap.put(GROUND_TYPE_CODES.get(TRACK_EVENT),new Color(160,114,19)); // TRACK piste (marron jaune)
		colorMap.put(GROUND_TYPE_CODES.get(GOOD_TRACK_EVENT),new Color(0,200,0)); // good track (marron fonc�)
