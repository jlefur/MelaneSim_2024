...
/** Style des agents "animaux". Définit une icône ou une ellipse pour chaque agent au lancement de la simulation en
 * fonction de son sexe et la fait varier suivant son âge.
...
public class C_StyleAgent implements StyleOGL2D<I_SituatedThing>, I_ConstantStringRodents, I_ConstantPNMC,
	...
	/** Initialise un gestionnaire d'images et enregistre les images qui seront utilis�es au cours de la simulation dans
	 * le factory */
	...
	public void initPNMC() {
		...
		factory.registerImage(NULL_CURRENT_ICON,selectImg.loadImage(NULL_CURRENT_ICON));
	}
	...
	/** Attribue une nouvelle "image" à un agent ou la modifie si besoin est, sinon renvoie le spatial en paramètre sans
	 * le modifier.
	...
	public VSpatial getVSpatial(I_SituatedThing agent, VSpatial spatial) {
		...
		// 2. change spatial only is requested
			...
			// 4. fetch the icon if IMAGE is on
				...
				String imageName = selectImg.getNameOfImage(agent);
				spatial = factory.getNamedSpatial(imageName);

				if(spatial==null){
					System.err.println("NULL VSpatial : "+agent.getClass().getName()+" / image = ["+imageName+"]");
				}
			}
	...
	public float getRotation(I_SituatedThing object) { return 0; }

	public int getBorderSize(I_SituatedThing object) { return 0; }

	public Color getBorderColor(I_SituatedThing object) { return null; }

	public String getLabel(I_SituatedThing object) {
		if(C_Parameters.VERBOSE){
			if(object instanceof A_Animal) return ((A_NDS)object).retrieveId()+"/"+((A_Animal)object).getCell1Target();
	...
	public float getLabelXOffset(I_SituatedThing object) { return 0; }

	public float getLabelYOffset(I_SituatedThing object) { return 0; }

	public ShapeFactory2D getFactory() { return factory; }

	public Position getLabelPosition(I_SituatedThing object) { return Position.SOUTH; }
