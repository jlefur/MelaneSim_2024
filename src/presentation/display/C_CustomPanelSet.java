...
public class C_CustomPanelSet implements IAction, ModelInitializer {
	...
	public static ArrayList<String> energyCurves = new ArrayList<String>();// used for energy graph (must be static to
	                                                                       // be invoked
	                                                                       // and cleared in
	public static ArrayList<String> populationCurves = new ArrayList<String>();// used for energy graph (must be static
	                                                                           // to be
	...
	public void initialize(Scenario scen, RunEnvironmentBuilder builder) {
		scen.addMasterControllerAction(new NullAbstractControllerAction<Object>() {
			...
			public void runInitialize(RunState runState, Context<?> context, Parameters runParams) {
				/*
				 * Initialisation des onglets: On crée un nouveau display puis on l'ajoute au registre des GUI de
				 * RunState. On recommence ces deux étapes autant de fois que l'on souhaite ajouter d'onglets.
				 */
				XYPlot plot;
				 ...
				 * l'intervalle demandé. La dernière variable ne peut pas être this à cause de la double implmentation.
				 * C'est pourquoi on utilise une variable action initialis�e avec this<br>
			...
			@Override
			public String toString() { return "PanelSetInitializer"; }
		});
	}
	...
	public void execute() {
		...
		if(A_Protocol.inspectorPopulation!=null){
			...
			for(String key:this.populationMap.keySet()){
				if(!key.contains("SoilCell") && !key.contains("LandPlot")){
					// if species not yet registered, create the XYSerie
					...
					// if (!key.equals("C_GenomeGerbillusNigeriae"))
					if(this.populationMap.get(key)>=1000)
					    curvePopSize.getChart().setData(key,RepastEssentials.GetTickCount(),this.populationMap.get(key)
					            /100);
					else
					    curvePopSize.getChart().setData(key,RepastEssentials.GetTickCount(),this.populationMap.get(
					            key));
				}
			}
		}
