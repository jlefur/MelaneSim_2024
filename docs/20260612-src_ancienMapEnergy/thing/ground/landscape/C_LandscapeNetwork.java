...
public class C_LandscapeNetwork extends C_LandscapeRodent {
	...
	public void buildGraphsFromLandPlots(TreeSet<C_LandPlot> aLandPlotList, String... graphTypeParam) {
		...
		String newGraphType = null; // le groundType du graph area lp : le lp o� le graphe ne doit sortir
		String newAreaLpGroundType = aLandPlotList.first().getPlotType();
		if (graphTypeParam.length != 0) // si graphTypeParam est donn�, cas des GNT ...
		...
		for (C_LandPlot aLandPlot : aLandPlotList) {
			...
			// V�rifier si le nouveau graphe � construire ne correspondrait pas � un ancien, qu'on doit mettre � jour
			...
			for (C_Graph oneOldGraph : this.graphList) { // Pour chaque ancien graphe
				// Si le groundType et le graphType correspondent � ceux de ce new landPlot, alors
				if (newAreaLpGroundType.equals(oneOldGraph.getGroundType()) && newGraphType.equals(oneOldGraph
					...
					for (C_SoilCellGraphed sc : oneOldGraph.getNodesList()) {
						if (aSCListOfaNewLandPlot.contains(sc)) {
							// Si oui alors on fait pointer le new graph sur un ancien graph une premi�re fois
							if (i == 0) { // suffisant pour une mise � jour par prolongement ou raccourcissement (et ensuite on
											// peut
								...
								i = 1;
							} // et s'il y a d'autres graphes candidats, ils vont pointer sur aGraph qui pointe d�j� sur un ancien
								// graphe, ce qui permet de relier plusieurs graphes

							else {
								graphsToRemoveList.add(oneOldGraph);// n�cessaire si le mis � jour peut relier des graphes
								if (C_Parameters.VERBOSE)
			...
			// D�truire les graphes obsol�tes
			...
			// Si i == 1, alors on ne construit pas un nouveau graphe mais on met � jour un ou des anciens
			// Et si i == 0, alors le new graphe ne correspon pas � un ancien
					...
					for (column = j - 1; column <= j + 1; column++) {
						if (0 <= line && line < nbTotalLine && 0 <= column && column < nbTotalColumn) {
							...
							if (tmp1.isOfGroundType(newGraphType) && tmp1 != tmp0 && nodeListTmp.contains(tmp1)) {
								// last condition est n�cessaire pour le cas GNT (un sc peut
								// etre du meme graphType mais n'appartenant pas � ce graphe)
	...
	/** Constuction ou mise � jour du graphe aGraphe
	...
	public TreeSet<C_LandPlot> identifyTypeLandPlots(String groundType) {
		...
		// I use HashSet car HashSet.contain() est le plus rapide (je le use bcp ici), je perds l'ordre mais je trie � la fin puis
		// je transforme en ArrayList (qui garde l'ordre d'insertion et est tr�s optimis�)
		...
		for (i = 0; i < dimension_Ucell.getWidth(); i++) { // Scan all the soilCellMatrix
			for (j = 0; j < dimension_Ucell.getHeight(); j++) {
				...
				if (sc0.isOfGroundType(groundType) && !newLandPlots.contains(sc0.getLandPlot(groundType))) {
					...
					// la fileDattente CONTIENT LES SOILCELLS QUI SONT D�J� TRAIT�ES :
					// Et dans cette list, les sc � partir de la position k
					// sont ceux dont les voisins ne sont pas encore tait�s
					waitingQueue.clear();
					waitingQueue.add(sc0); // Seul sc0 est trait� en ce moment
					k = 0;
							...
							for (y = y0 - 1; y <= y0 + 1; y++) { // avec ces 2 boucles j'accede � tous les 8 voisins de sc0
								if (!(x == x0 && y == y0) && (0 <= x && x < dimension_Ucell.getWidth() && 0 <= y
									...
									if (scI.isOfGroundType(groundType) && // si c'est du meme ground type que sc0 et
										...
										waitingQueue.add(scI); // et l'ajoute dans la file d'attente pour �tudier � son tour
